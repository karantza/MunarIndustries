...
namespace Kasselblad {
  ...
  public class CameraFilter {
    ...
    public static Material LoadShaderFile(string fileName) {
      try {
				string path = KSPUtil.ApplicationRootPath.Replace(@"\", "/") + "/GameData/MunarIndustries/Shaders/" + fileName;
        ...
        return new Material(shader);
				...
				Debug.Log(string.Format("Kasselblad: LoadShaderFile exception: {0}", ex.Message));
      }
      return null;
    }
    ...
    public static Texture2D LoadTextureFile(string fileName) {
      try {
        string path;
				...
				path = KSPUtil.ApplicationRootPath.Replace(@"\", "/") + "/GameData/MunarIndustries/Textures/" + fileName;
        ...
        Debug.Log(string.Format("Kasselblad: LoadTextureFile exception: {0}", ex.Message));
      }
      return null;
    }
  }
